...
// https://github.com/avrj/slack-clone/blob/master/src/server/socket.js
  ...
  models.Channel.findOne({ name: channelToJoin }, (err, channel) => {
    if (channel) {
      models.User.findOne({
        ...
        .then(user => {
          if (user) {
            const errorMsg = `error joining channel: ${socket.request.user} is already joined to ${channelToJoin}`;
            debug(errorMsg);
        ...
        .then(() => {
          debug(
            `${socket.request.user} (${socket.id}) joined channel ${channelToJoin}`
          );
        });
    } else {
      connectedClientsForUser.forEach(socketInstance => {
...
const handleMsg = socket => data => {
  ...
  newMessage
    ...
    .then(() =>
      debug(
        `${socket.request.user} (${socket.id}) sent message ${data.msg} to channel ${data.room}`
      )
    )
...
const handlePrivateMsg = (socket, io, connectedClientsForUser) => data => {
  ...
  debug(
    `${socket.request.user} (${socket.id}) sent private message ${data.msg} to ${data.to}`
  );
  ...
  if (data.to !== socket.request.user) {
    ...
    passportSocketIo
      ...
      .forEach(async socketInstance => {
        socketInstance.emit(events.privateMsg, {
          from: socket.request.user,
          msg: data.msg,
        });
        const chatData = new models.PrivateChat({
          fromUser: socket.request.user,
          toUser: data.to,
          msg: data.msg,
        });
        await chatData.save();
...
module.exports = (config, io) => {
  io.sockets.on("connection", socket => {
    socket.emit("test", socket.client.id)
    socket.on("test1", () => {
      console.log(`===========================`)
      console.log(`===========================`)
      console.log(`===========================`)
